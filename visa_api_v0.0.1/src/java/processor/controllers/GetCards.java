 ...
 * @author kjacone
 */
public class GetCards  extends MustHave {

      @Override
    public String sendData(String incoming_request) {
        JSONObject js = new JSONObject();
        try {
              js = new CommonFunctions().uploadToServer(Config.GET_CARDS,incoming_request);
    ...
    public String createRequestBody(JSONObject incoming_request) throws JSONException {
     ...
     pam.put("mobileNr","255767144609");
//     pam.put("mobileNr",incoming_request.get("phoneNumber"));
//     pam.put("imei",incoming_request.get("imsi"));
//     pam.put("token",incoming_request.get("token"));
//     pam.put("deviceInfo",incoming_request.sssssssgetJSONObject("deviceInfo"));
    ...
    public String decodeResponseBody(String incoming_response) {
        ...
        try {
            ...
            if (rspCode.equalsIgnoreCase("00")) {
                  jess1.put("f39", "00");
                jess1.put("f48", jess.getString("CardData"));
              
               
            } else {
                jess1.put("f48",  jess.getString("Message"));
